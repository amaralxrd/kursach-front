...
export const getTickets = () => async dispatch => {
    await axios.get(`http://amaralxrd.pythonanywhere.com/api/ticket/?limit=1000`)
        .then((res) => dispatch(ticketSlice.actions.setTickets(res.data.results)))
}

export const createTicket = (ticket) => async (dispatch) => {
    await axios.post('http://amaralxrd.pythonanywhere.com/api/ticket/', ticket, {
        headers: {
...
export const editTicket = (ticket, id) => async (dispatch) => {
    ...
    await axios.put(`http://amaralxrd.pythonanywhere.com/api/ticket/${id}/`, ticket, {
        headers: {
            "Content-type": "application/json"
        }
    })
...
export const deleteTicket = (id) => async (dispatch) => {
    await axios.delete(`http://amaralxrd.pythonanywhere.com/api/ticket/${id}/`)
